...
export default function MessengerApp() {
  ...
  ...chat,
  chatId: chat.chatId || chat.id, // используем chat.id если chatId нет
  ...
  const query = `?name=${encodeURIComponent(chat.receiveUserName)}&avatar=${encodeURIComponent(chat.receiveUserImage || "")}`;
  router.push(`/chats/${chat.chatId}${query}`);
  setShowSidebar(false);
};

  const handleUserSelect = (user) => {
  router.push(`/chats/new?userId=${user.id}&name=${encodeURIComponent(user.fullName)}`);
  setShowModal(false);
};
  ...
  return (
    <div className="flex h-screen bg-gray-100 antialiased text-gray-800">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 bg-blue-500 rounded-full flex items-center justify-center text-white font-bold">
              {CURRENT_USER_ID.slice(0, 2).toUpperCase()}
            </div>
            <span className="font-semibold text-gray-700 text-lg">Majid</span>
          </div>
          <button
      ...
      {/* Модалка для выбора пользователя */}
      {showModal && (
  <div
