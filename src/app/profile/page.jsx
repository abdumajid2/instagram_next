...
const BASE_URL = "http://37.27.29.18:8003/images"; // базовый URL для изображений с сервера
...
const getImageSrc = (fileName) => {
  ...
  // если fileName уже абсолютный URL
  if (fileName.startsWith("http")) return fileName;
  // иначе строим полный путь
    ...
    <div className="sm:max-w-[640px] ml-[100px] mt-5">
      ...
      <section className="w-full flex items-center justify-between h-[160px]">
        <Image
          ...
          className="h-[160px] w-[160px] object-cover  rounded-[50%]"
        ...
        {isMenuLoading && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div className="w-[500px] flex flex-col gap-[30px] p-[50px] rounded-4xl bg-white">
              ...
              <p className="text-2xl font-bold cursor-pointer" onClick={() => {setShowQRCode(true); setIsMenuLoading(false)}}>Qr code</p>
              <Link href={'/notification'}><p className="text-2xl font-bold">Notification</p></Link>
              <Link href={'/setting'}><p className="text-2xl font-bold">Settings and privacy</p></Link>
              <p className="text-red-600 text-2xl font-bold cursor-pointer"
                onClick={() => {
  ...
  <button className="bg-[#F3F4F6] w-[105px] h-[40px] rounded-xl flex items-center justify-center">
    Edit profile
          ...
          <div className="flex w-full items-start gap-7">
            ...
            <p className="text-[14px] font-[600] cursor-pointer"
                ...
                .map(user => (
                  <div key={user.userShortInfo.userId} className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3">
                      <Image
                        src={getImageSrc(user.userShortInfo.userPhoto)}
                        width={40}
                        height={40}
      ...
      <section className="w-full bg my-[30px] p-[20px] border-t flex items-center justify-center space-x-10 border-gray-400">
        <article onClick={() => setActiveTab("posts")}
          className={`cursor-pointer font-[500] flex items-center gap-[10px] ${activeTab === "posts" ? "text-[#2563EB] border-t-2 border-[#2563EB]" : "text-[#64748B]"}`}>
          <TiThSmallOutline className="text-xl" />
          <p>Posts</p>
        </article>

        <article onClick={() => setActiveTab("saved")}
          className={`cursor-pointer font-[500] flex items-center gap-[10px] ${activeTab === "saved" ? "text-[#2563EB] border-t-2 border-[#2563EB]" : "text-[#64748B]"}`}>
          <FaRegBookmark className="text-xl" />
          <p>Saved</p>
        </article>

        <article onClick={() => setActiveTab("tagged")}
          className={`cursor-pointer font-[500] flex items-center gap-[10px] ${activeTab === "tagged" ? "text-[#2563EB] border-t-2 border-[#2563EB]" : "text-[#64748B]"}`}>
